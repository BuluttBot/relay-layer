    ...
    <div className="absolute inset-0 z-30 bg-bg-primary/95 backdrop-blur-sm flex flex-col">
      <div className="flex items-center justify-between px-6 py-4 border-b border-border-subtle">
        <h2 className="text-lg font-bold text-text-primary flex items-center gap-2">
          ...
          Chat Viewer
          <span className="text-xs text-text-tertiary font-normal ml-2">powered by Telegram</span>
        </h2>
        <button onClick={() => close('chatViewerOpen')} className="p-2 rounded-lg hover:bg-bg-tertiary transition-fast">
          ...
          messages.map(msg => {
            ...
            return (
              <div key={msg.id} className={`flex ${isOrchestrator ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] rounded-2xl px-4 py-3 ${isOrchestrator ? 'bg-accent-purple/20 rounded-tr-sm' : 'bg-bg-secondary border border-border-subtle rounded-tl-sm'}`}>
