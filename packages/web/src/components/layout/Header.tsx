...
import { Radio, BarChart3, MessageCircle, Activity, Users, LogOut, ChevronDown } from 'lucide-react';
...
export default function Header() {
  ...
  return (
    <header className="h-14 border-b border-border-subtle bg-bg-secondary flex items-center px-4 gap-4 shrink-0">
      ...
      <div className="relative group">
        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-bg-tertiary hover:bg-bg-surface text-sm text-text-primary transition-fast">
          {activeProject?.icon} {activeProject?.name || 'Select Project'}
      ...
      <div className="flex items-center gap-1">
        ...
        <button onClick={() => toggle('agentsSidebarOpen')} className="p-2 rounded-lg hover:bg-bg-tertiary text-text-secondary hover:text-accent-teal transition-fast" title="Agents">
          <Users size={18} />
        </button>
